...
// ===============================
function notificarTareasPendientes() {
    const tareas = obtenerTareas();
    const hoy = new Date();
    ...
    tareas.forEach(t => {
        ...
        if (diff < 0) vencidas++;
        else if (diff === 0) hoyCount++;
        else if (diff === 1) manana++;
    });
    
    if (vencidas > 0) {
    mostrarToast("⚠️ Tareas vencidas", `Tienes ${vencidas} tareas atrasadas.`);
    enviarNotificacion("⚠️ Tareas vencidas", `Tienes ${vencidas} tareas atrasadas.`);
    }

    if (hoyCount > 0) {
    mostrarToast("⏰ Para hoy", `${hoyCount} tareas vencen hoy.`);
    enviarNotificacion("⏰ Tareas para hoy", `${hoyCount} tareas vencen hoy.`);
    }

    if (manana > 0) {
    mostrarToast("📅 Para mañana", `${manana} tareas vencen mañana.`);
    enviarNotificacion("📅 Tareas para mañana", `${manana} tareas vencen mañana.`);
    }
}


// Ejecutar al cargar
document.addEventListener("DOMContentLoaded", notificarTareasPendientes);
...
// Marcar automáticamente el link activo del navbar
document.addEventListener("DOMContentLoaded", () => {
...
function actualizarResumenTareas() {
    ...
    const compEl = document.getElementById("tareasCompletadas");
    const pendEl = document.getElementById("tareasPendientes");

    if (!totalEl || !compEl || !pendEl) return;

    const { total, completadas, pendientes } = obtenerEstadisticas();

    totalEl.textContent = total;
    compEl.textContent = completadas;
    pendEl.textContent = pendientes;
}
